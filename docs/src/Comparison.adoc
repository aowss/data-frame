...
==== Selecting `Series` from a `DataFrame`
...
df[["Age", "Sex"]] <2>
----
<1> When selecting a single column the result is a `Series`.
<2> When selecting multiple columns, you need to pass an array of labels.
...
.Joinery
----
List<Integer> ages = dataFrame.col("Age"); <1>
DataFrame columns = dataFrame.retain("Age", "Sex"); <2>
----
<1> When selecting a single column the result is a `List`.
<2> When selecting multiple columns, you need to use the `retain` method and the result is a `DataFrame`.
...
----
<1> The result is always a `Table`.
